...
function viewAllRoles() {
    const sql = `SELECT * FROM roletable`;
    db.query(sql, (err, result) => {
        if (err) {
            res.status(500).json({ error: err.message })
            return;
...
function viewAllEmployees() {
    const sql = `SELECT employee.id,
                employee.first_name,
                employee.last_name,
                roletable.title AS job_title,
                department.department_name,
                roletable.salary,
                CONCAT(manager.first_name, ' ' ,manager.last_name) AS manager
                FROM employee
                LEFT JOIN roletable ON employee.roletable_id = roletable.id
                LEFT JOIN department ON roletable.department_id = department.id
...
function addRole() {
    inquirer.prompt([
        ...
        db.query("INSERT INTO roletable (title, salary, department_id) VALUES (?, ?, ?)", [response.title, response.salary, response.department_id], function (err, data) {
            if (err) throw err;
...
function addEmployee() {
    inquirer.prompt([
        ...
        db.query("INSERT INTO employee (first_name, last_name, roletable_id, manager_id) VALUES (?, ?, ?, ?)", [response.first_name, response.last_name, response.roletable_id, response.manager_id], function (err, data) {
            if (err) throw err;
...
function updateEmployeeRole() {
    inquirer.prompt([
        ...
        db.query("UPDATE employee SET roletable_id = ? WHERE first_name = ?", [response.roletable_id, response.first_name], function (err, data) {
            if (err) throw err;
            console.log('The new role entered has been added successfully to the database.');
...
function deleteRole() {
    inquirer.prompt([
        ...
        db.query("DELETE FROM roletable WHERE id = ?", [response.roletable_id], function (err, data) {
            if (err) throw err;
